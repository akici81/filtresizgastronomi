    ...
    <Layout>
      ...
      <div style={{ background: 'linear-gradient(180deg, #1a0000 0%, #0d0d0d 100%)', padding: '60px 0 40px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
          ...
          <p style={{ fontSize: 16, color: COLORS.dim, margin: 0 }}>{total} restoran keşfedilmeyi bekliyor</p>
        </div>
      </div>
      ...
      <div style={{ borderBottom: `1px solid ${COLORS.border}`, background: '#0d0d0d', position: 'sticky', top: 0, zIndex: 30 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', display: 'flex', gap: 12, alignItems: 'center', height: 56, flexWrap: 'wrap' }}>
          <input type="text" placeholder="Restoran ara..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
            style={{ flex: 1, minWidth: 180, background: 'rgba(255,255,255,0.06)', border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '8px 14px', color: COLORS.white, fontSize: 13, outline: 'none' }} />
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
        {loading ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 24 }}>
            {[...Array(12)].map((_, i) => <div key={i} style={{ height: 340, borderRadius: 12, background: COLORS.card, animation: 'pulse 1.5s infinite' }} />)}
          </div>
              ...
              <div key={r.id} onClick={() => router.push(`/restaurant/${r.slug}`)}
                ...
                <div style={{ height: 200, overflow: 'hidden', position: 'relative' }}>
                  ...
                  <div style={{ position: 'absolute', top: 12, left: 12, display: 'flex', gap: 6 }}>
                    {r.is_premium && <Badge color={COLORS.red}>Premium</Badge>}
                    {r.is_verified && <Badge color="#10b981">✓ Doğrulandı</Badge>}
                  </div>
                  {r.price_range && (
                    <div style={{ position: 'absolute', top: 12, right: 12, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', padding: '4px 10px', borderRadius: 20, fontSize: 12, color: '#f59e0b', fontWeight: 700 }}>
                ...
                <div style={{ padding: 20 }}>
                  ...
                  <div style={{ display: 'flex', gap: 12, marginBottom: 8 }}>
                    {r.cities?.name && <span style={{ fontSize: 12, color: COLORS.red }}>📍 {r.cities.name}</span>}
                    {r.cuisine_type && <span style={{ fontSize: 12, color: COLORS.dim }}>{r.cuisine_type}</span>}
                  </div>
                  {r.short_description && <p style={{ margin: '0 0 16px', fontSize: 13, color: COLORS.dim, lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{r.short_description}</p>}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    {r.average_rating > 0
                      ? <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <span style={{ color: '#f59e0b' }}>★</span>
                          <span style={{ fontSize: 13, fontWeight: 600 }}>{Number(r.average_rating).toFixed(1)}</span>
                          <span style={{ fontSize: 11, color: COLORS.muted }}>({r.reviews_count})</span>
                        </div>
                      : <span style={{ fontSize: 12, color: COLORS.muted }}>Değerlendirme yok</span>}
