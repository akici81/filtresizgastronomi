  ...
  if (!dish) return (
    <Layout>
      <div style={{ textAlign: 'center', padding: '120px 24px' }}>
        ...
        <button onClick={() => router.push('/dishes')} style={{ marginTop: 16, background: COLORS.red, border: 'none', color: COLORS.white, padding: '10px 24px', borderRadius: 6, cursor: 'pointer' }}>Tüm Yemekler</button>
      </div>
    </Layout>
  );
    ...
    <Layout>
      ...
      <div style={{ position: 'relative', height: 480, overflow: 'hidden' }}>
        {dish.image_url
          ...
          : <div style={{ width: '100%', height: '100%', background: 'linear-gradient(180deg, #2a0000, #0d0d0d)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 80 }}>🍽</div>}
          ...
          <div>
            ...
            {dish.recipe && (
              <div style={{ marginBottom: 40, paddingBottom: 40, borderBottom: `1px solid ${COLORS.border}` }}>
                ...
                <div style={{ fontSize: 15, color: COLORS.dim, lineHeight: 1.8 }} dangerouslySetInnerHTML={{ __html: dish.recipe }} />
              </div>
            )}
            ...
            <div>
              ...
              {reviews.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 0', color: COLORS.muted }}>Henüz değerlendirme yok. İlk sen değerlendir!</div>
              ) : (
              ...
              <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 24 }}>
                ...
                <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                  {ingredients.map((ing, i) => (
                    <li key={i} style={{ padding: '8px 0', borderBottom: `1px solid ${COLORS.border}`, fontSize: 13, color: COLORS.dim, display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ color: COLORS.red, fontSize: 8 }}>●</span>
