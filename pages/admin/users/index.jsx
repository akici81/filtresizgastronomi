  ...
  return (
    <AdminLayout title="Kullanıcılar">
      ...
      <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 8, overflow: 'hidden' }}>
        ...
        {loading ? (
          [...Array(8)].map((_, i) => (
            <div key={i} style={{ height: 60, borderBottom: `1px solid ${COLORS.border}`, animation: 'pulse 1.5s infinite' }} />
          ))
        ) : filtered.length === 0 ? (
          ...
          filtered.map((user) => (
            ...
            >
              ...
              {/* Stats */}
              <span style={{ fontSize: 12, color: COLORS.dim }}>{user.reviews_count || 0}</span>
              <span style={{ fontSize: 12, color: COLORS.dim }}>{user.favorites_count || 0}</span>

              {/* Actions */}
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                <ActionBtn
                  onClick={() => router.push(`/admin/users/${user.id}`)}
                  color={COLORS.dim} title="Detay"
                >
                  ✎
                </ActionBtn>
              </div>
