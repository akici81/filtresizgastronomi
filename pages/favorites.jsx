  ...
  return (
    <Layout>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '48px 24px' }}>
        ...
        {/* İçerik */}
        {loading ? (
          <div style={{ textAlign: 'center', padding: '80px 0', color: COLORS.muted }}>Yükleniyor...</div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <button
              ...
              style={{ background: COLORS.red, border: 'none', color: COLORS.white, padding: '10px 24px', borderRadius: 6, cursor: 'pointer', fontSize: 13, fontWeight: 700 }}
            >
              Keşfet
            </button>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
            {filtered.map(fav => {
              ...
              return (
                <div
                  key={fav.id}
                  style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 10, overflow: 'hidden', position: 'relative', transition: 'transform 0.15s, border-color 0.15s' }}
                ...
                >
                  ...
                  >
                    {img ? (
                      ...
                      <div style={{ width: '100%', height: '100%', background: 'rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 40 }}>
                        {typeIcon}
                      </div>
                    )}
                    {/* Tip badge */}
                    <div style={{ position: 'absolute', top: 10, left: 10, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', padding: '3px 8px', borderRadius: 4, fontSize: 11, color: COLORS.dim, display: 'flex', alignItems: 'center', gap: 4 }}>
                      <span>{typeIcon}</span>
                      <span>{type === 'dish' ? 'Yemek' : type === 'restaurant' ? 'Restoran' : type === 'chef' ? 'Şef' : 'Makale'}</span>
                    </div>
                    {/* Rating */}
                    {rating > 0 && (
                      <div style={{ position: 'absolute', top: 10, right: 10, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', padding: '3px 8px', borderRadius: 4, fontSize: 12, color: '#f59e0b', fontWeight: 700 }}>
                        ★ {rating.toFixed(1)}
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ padding: '12px 14px 14px' }}>
                    <h3
                      onClick={() => router.push(href)}
                      style={{ margin: '0 0 4px', fontSize: 14, fontWeight: 700, lineHeight: 1.4, cursor: 'pointer', color: COLORS.white }}
                    >
                      {title}
                    </h3>
                    {subtitle && (
                      <p style={{ margin: '0 0 8px', fontSize: 12, color: COLORS.muted }}>{subtitle}</p>
                    )}
                    {fav.collection_name && (
                      <div style={{ fontSize: 11, color: COLORS.dim, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 4 }}>
                        <span>📁</span> {fav.collection_name}
                      </div>
                    )}
                    {fav.note && (
                      <p style={{ margin: '0 0 8px', fontSize: 12, color: COLORS.dim, fontStyle: 'italic', lineHeight: 1.5 }}>"{fav.note}"</p>
                    )}
