    ...
    <Layout>
      ...
      <div style={{ background: 'linear-gradient(180deg, #1a0000 0%, #0d0d0d 100%)', padding: '60px 0 40px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
          ...
          <p style={{ fontSize: 16, color: COLORS.dim, margin: 0 }}>Türkiye'nin usta mutfak ustalarını tanıyın</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
        {loading ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 24 }}>
            {[...Array(8)].map((_, i) => <div key={i} style={{ height: 320, borderRadius: 12, background: COLORS.card, animation: 'pulse 1.5s infinite' }} />)}
          </div>
        ) : chefs.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 24 }}>
            {chefs.map(chef => (
              <div key={chef.id} onClick={() => router.push(`/chef/${chef.slug}`)}
                style={{ cursor: 'pointer', background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 12, overflow: 'hidden', transition: 'transform 0.2s, border-color 0.2s', textAlign: 'center' }}
                ...
                <div style={{ height: 220, overflow: 'hidden', position: 'relative' }}>
                  {chef.image_url
                    ...
                    : <div style={{ width: '100%', height: '100%', background: 'linear-gradient(135deg, rgba(232,0,13,0.2), rgba(232,0,13,0.05))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <div style={{ width: 80, height: 80, borderRadius: '50%', background: COLORS.red, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, fontWeight: 700 }}>{chef.name[0]}</div>
                      </div>}
                  {chef.is_featured && (
                    <div style={{ position: 'absolute', top: 12, right: 12, background: COLORS.red, padding: '2px 8px', borderRadius: 20, fontSize: 9, fontWeight: 700, letterSpacing: '0.06em' }}>ÖNE ÇIKAN</div>
                  )}
                </div>
                <div style={{ padding: 20 }}>
                  <h3 style={{ margin: '0 0 4px', fontSize: 17, fontWeight: 700 }}>{chef.name}</h3>
                  {chef.title && <div style={{ fontSize: 12, color: COLORS.red, marginBottom: 8 }}>{chef.title}</div>}
                  {chef.cities?.name && <div style={{ fontSize: 12, color: COLORS.dim, marginBottom: 10 }}>📍 {chef.cities.name}</div>}
                  {chef.short_bio && <p style={{ margin: 0, fontSize: 12, color: COLORS.muted, lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{chef.short_bio}</p>}
                </div>
              </div>
            ))}
          </div>
