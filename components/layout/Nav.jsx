...
import { useTema, TemaToggle } from '../../lib/ThemeContext';
...
export default function Nav({ transparent = false }) {
  ...
  return (
    <nav style={{
      ...
      background: isTransparent ? 'transparent' : 'var(--nav-bg)',
      ...
      <div onClick={() => router.push('/')} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 16, fontWeight: 900, letterSpacing: '0.08em', color: 'var(--text)' }}>FİLTRESİZ</span>
        <span style={{ fontSize: 16, fontWeight: 300, color: 'var(--red)' }}>GASTRONOMİ</span>
      </div>
        ...
        {menuItems.map(item => (
          <span
            ...
            style={{
              ...
              color: router.pathname === item.href ? 'var(--text)' : 'var(--dim)',
              borderBottom: router.pathname === item.href ? '1px solid var(--red)' : '1px solid transparent',
              paddingBottom: 2, transition: 'color 0.15s',
            }}
            onMouseEnter={e => e.target.style.color = 'var(--text)'}
            onMouseLeave={e => e.target.style.color = router.pathname === item.href ? 'var(--text)' : 'var(--dim)'}
          >
            {item.label}
          </span>
        ))}
        ...
        {user ? (
          <div ref={menuRef} style={{ position: 'relative' }}>
            ...
            <div
              ...
              style={{
                ...
                overflow: 'hidden', border: `2px solid ${menuOpen ? 'var(--red)' : 'transparent'}`,
              ...
              <div style={{
                ...
                <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--red)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: '#fff', overflow: 'hidden', flexShrink: 0 }}>
                      {profile?.avatar_url
                        ? <img src={profile.avatar_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        : avatarLetter}
                    </div>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{profile?.full_name || 'Kullanıcı'}</div>
                      <div style={{ fontSize: 11, color: 'var(--muted)' }}>@{profile?.username || 'user'}</div>
                    </div>
                  </div>
                  {profile?.role && (
                    <div style={{ marginTop: 8, fontSize: 9, color: 'var(--red)', letterSpacing: '0.1em', fontWeight: 700 }}>
                ...
                {isStaff && (
                  <>
                    ...
                    <DropdownItem icon="🛠" label="Admin Panel" onClick={() => { router.push('/admin'); setMenuOpen(false); }} color="var(--red)" />
                  </>
                )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button
              ...
              style={{ background: 'transparent', border: 'none', color: 'var(--dim)', fontSize: 11, letterSpacing: '0.1em', cursor: 'pointer', padding: '8px 12px' }}
            >
              GİRİŞ
            </button>
            <button
              onClick={() => router.push('/register')}
              style={{ background: 'var(--red)', border: 'none', color: '#fff', fontSize: 10, letterSpacing: '0.1em', cursor: 'pointer', padding: '10px 18px', borderRadius: 4, fontWeight: 700 }}
            >
              KAYIT OL
            </button>
          </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        color: color || (hover ? 'var(--text)' : 'var(--dim)'),
